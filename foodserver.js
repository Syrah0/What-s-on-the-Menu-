...
ap.post("/restaurant-con-plato",rpg.multiSQL({
    dbcon: conString,
    postReqData: ["name"],
    sql: "SELECT DISTINCT sponsor FROM menu WHERE sponsor <> '?' AND id IN (SELECT menu_id FROM menuPage WHERE id IN (SELECT menu_page_id FROM MenuItem WHERE dish_id IN (SELECT id FROM dish WHERE name LIKE $1))) ORDER BY sponsor limit 10;",
    sqlParams: [rpg.sqlParam("post","name")]
}));
